...
# Create AVML bucket
resource "google_storage_bucket" "bsides-avml-bucket" {
  name          = "bsides-avml-bucket"
  location      = "US"
  force_destroy = true
...
resource "google_storage_bucket_object" "memory-dump-folder" {
  ...
  bucket        = "${google_storage_bucket.bsides-avml-bucket.name}"
}
...
data "template_file" "avml_startup_script" {
  ...
  vars = {
    BUCKET_NAME = "${google_storage_bucket.bsides-avml-bucket.name}"
...
output "gcp_avml_bucket" {
  value = google_storage_bucket.bsides-avml-bucket.name
}
